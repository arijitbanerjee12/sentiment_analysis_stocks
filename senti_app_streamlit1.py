...
def main_func(company ,ticker, days):
  ...
  company_ticker, company_link = 'None' , 'na'
...
options = st.sidebar.selectbox("Which Dashboard", ['Sentiment Analysis'])
...
if options == 'Sentiment Analysis':
    ...
    st.write("This app is going to help you in getting the sentiments attached with news ")
    st.write('Wea all know that the stock prices of the company depends on the news about the company')
    st.write ("if the news is positive then the stock pricesa re more likely to go up")
    st.write("and wise versa. So this app is going to help you to figure out if the newsa re positive or negative ")
    st.write("When you give us the Company ticker(symbol of the company) we are going to search the most relevent  ")
    st.write('news about the company and then apply Machinne learning algorithms to figure out if those news are ')
    st.write('positive or negative then give you a overall sentiments for all the news ')
    st.write('Which is going to help you to decide if the share price will go up or down')
    
    st.title("    ")
    
    #company = st.text_input('Please enter the Company and press enter ex - Infosys')
    ticker = st.text_input('Please enter the Ticker of the company and press enter ex for  Infosys put  INFY ')
    days = st.slider('Consider news for these many preceeding days ', min_value=1, max_value=14)
